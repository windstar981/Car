...
(() => {
/*!*********************************************************!*\
  !*** ./platform/interest/src/resources/js/insurance.js ***!
  \*********************************************************/
/* provided dependency */ var $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js");
$(document).ready(function () {
  var optionInsurance = 1;
  ...
  function handlingDataTabVoluntary() {
    ...
    for (var i = 0; i < arrVoluntary.length; i++) {
  ...
  function getAdjustment(year) {
    for (var i = 0; i < priceLost.length; i++) {
  ...
  function getTotalMonth(data) {
    if (Number(data['fromYear']) === Number(data['toYear'])) {
      ...
      var fromMonth = 12 - Number(data['fromMonth']) + 1;
      var toMonth = Number(data['toMonth']);
      var midMonth = Number(data['toYear']) - Number(data['fromYear']) <= 1 ? 0 : (Number(data['toYear']) - Number(data['fromYear']) - 1) * 12;
      var sumMonth = Number(fromMonth) + Number(toMonth) + Number(midMonth);
      return sumMonth;
    }
  }
  ...
  function solveStage(arrMust) {
    ...
    var sumMonthAfter = 0;
    var html = "";
    arrMust.forEach(function (data) {
      var moneyStep = 0;
      sumMonth += Number(getTotalMonth(data)); //tính tiền giai đoạn

      if (data['type'] === 'must') {
        if (Number(data['fromYear']) === Number(data['toYear'])) {
          sumMoney += Number(data['money']) * (Number(data['toMonth']) - Number(data['fromMonth']) + 1) * Number(getAdjustment(data['fromYear']));
          ...
          html += getHtmlElement(data['fromMonth'], data['fromYear'], data['toMonth'], data['toYear'], Number(data['toMonth']) - Number(data['fromMonth']) + 1, data['money'], getAdjustment(data['fromYear']));
        } else {
          //tính tiền năm đầu
          ...
          html += getHtmlElement(data['fromMonth'], data['fromYear'], 12, data['fromYear'], 12 - Number(data['fromMonth']) + 1, data['money'], getAdjustment(data['fromYear'])); //tính tiền giữa các năm

          for (var i = Number(data['fromYear']) + 1; i < Number(data['toYear']); i++) {
          ...
          html += getHtmlElement(1, data['toYear'], data['toMonth'], data['toYear'], Number(data['toMonth']), data['money'], getAdjustment(data['toYear']));
        }
      } //tính tiền thai sản
      else {
        //xét trường hợp trùng năm
        if (Number(data['fromYear']) === Number(data['toYear'])) {
          sumMoney += Math.floor(findMoney(arrMust, data) * (Number(data['toMonth']) - data['fromMonth'] + 1) * getAdjustment(data['fromYear']));
          ...
          html += getHtmlElementMaternity(data['fromMonth'], data['fromYear'], data['toMonth'], data['toYear'], Number(data['toMonth']) - Number(data['fromMonth']) + 1, findMoney(arrMust, data), getAdjustment(data['fromYear']));
        } else {
          //tính tiền năm đầu
          ...
          html += getHtmlElementMaternity(data['fromMonth'], data['fromYear'], 12, data['fromYear'], 12 - Number(data['fromMonth']) + 1, findMoney(arrMust, data), getAdjustment(data['fromYear'])); //tính tiền năm cuối
          ...
          html += getHtmlElementMaternity(1, data['toYear'], data['toMonth'], data['toYear'], Number(data['toMonth']), findMoney(arrMust, data), getAdjustment(data['toYear']));
        }
      }
    });
    var totalYear = Math.floor(Number(sumMonth) / 12);
    var totalMonth = Number(sumMonth) % 12;
    var totalMoneyBefore = 0;
    var totalMoneyAfter = 0;
    sumMoney = Math.floor(sumMoney);
    $('.total-all').text(formatMoney(sumMoney));
    var bqtl = Math.floor(Number(sumMoney) / Number(sumMonth));
    ...
    if (totalMoneyBefore != 0) {
      $('.before-2014').text("M\u1EE9c h\u01B0\u1EDFng BHXH m\u1ED9t l\u1EA7n \u0111\u1ED1i v\u1EDBi th\u1EDDi gian \u0111\xF3ng BHXH tr\u01B0\u1EDBc 2014 = " + formatMoney(totalMoneyBefore) + " (VN\u0110)");
    }

    if (totalMoneyAfter) {
      $('.after-2014').text("M\u1EE9c h\u01B0\u1EDFng BHXH m\u1ED9t l\u1EA7n \u0111\u1ED1i v\u1EDBi th\u1EDDi gian \u0111\xF3ng BHXH t\u1EEB 2014 tr\u1EDF \u0111i = " + formatMoney(totalMoneyAfter) + " (VN\u0110)");
    }
  ...
  function getHtmlStateSupportBefore(m1, m2, m3, res) {
    return "\n              0.22 x 700,000 (VN\u0110) x 0.3% x " + m1 + " th\xE1ng<br>" + "0.22 x 700,000 (VN\u0110) x 0.25% x " + m2 + " th\xE1ng<br>" + "0.22 x 700,000 (VN\u0110) x 0.1% x " + m3 + " th\xE1ng<br> = " + formatMoney(res.toString());
  }

  function getHtmlStateSupportAfter(m1, m2, m3, res) {
    return "\n              0.22 x 1,500,000 (VN\u0110) x 0.3% x " + m1 + " th\xE1ng <br>" + "0.22 x 1,500,000 (VN\u0110) x 0.25% x " + m2 + " th\xE1ng <br>" + "0.22 x 1,500,000 (VN\u0110) x 0.1% x " + m3 + " th\xE1ng<br> = " + formatMoney(res.toString());
  }
  ...
  function solveVoluntary(arr) {
    ...
    arr.forEach(function (data) {
      ...
      if (Number(data['fromYear']) === Number(data['toYear'])) {
        ...
        html += getHtmlElement(1, data['toYear'], data['toMonth'], data['toYear'], Number(data['toMonth']), data['money'], getAdjustment(data['toYear'])); //tính số tháng nhà nước hỗ trợ
      }
    });
    var sumPoorBefore = 0;
    var sumNearPoorBefore = 0;
    var sumOtherBefore = 0;
    var sumPoorAfter = 0;
    var sumNearPoorAfter = 0;
    var sumOtherAfter = 0;
    arr.forEach(function (data) {
    ...
    var totalStateBefore = 0;
    var totalStateAfter = 0;
    totalStateBefore = 0.22 * 700000 * 0.3 * sumPoorBefore + 0.22 * 700000 * 0.25 * sumNearPoorBefore + 0.22 * 700000 * 0.1 * sumOtherBefore;
    totalStateAfter = 0.22 * 1500000 * 0.3 * sumPoorAfter + 0.22 * 1500000 * 0.25 * sumNearPoorAfter + 0.22 * 1500000 * 0.1 * sumOtherAfter;
    ...
    $('.detail-insurance').append(html);
    var totalStateSupport = Number(totalStateBefore) + Number(totalStateAfter);
    var total_bhxh = Number(totalMoneyBefore) + Number(totalMoneyAfter) - totalStateSupport;
  ...
  function solveMix(arrMust) {
    ...
    var arr = [];
    arrMust.forEach(function (element) {
    ...
    $('.money-support-before').html(getHtmlStateSupportBefore(sumPoorBefore, sumNearPoorBefore, sumOtherBefore, totalStateBefore));
    $('.money-support-after').html(getHtmlStateSupportAfter(sumPoorAfter, sumNearPoorAfter, sumOtherAfter, totalStateAfter)); //end tính ngày
